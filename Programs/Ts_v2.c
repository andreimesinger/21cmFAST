#include "heating_helper_progs_v2.c"
...
// New in v1.3(?): To calculate follapse fraction for new parametrization
...
int main(int argc, char ** argv){
  ...
  float F_STAR10,ALPHA_STAR,M_TURN,T_AST,Splined_Fcoll; // New in v1.3(?)
 ...
 */
 ...
 //set the minimum ionizing source mass //*** New in v1.3(?) have to be changed to be as Mturn/50 ?
...
//(FgtrM(REDSHIFT, FMAX(TtoM(REDSHIFT, X_RAY_Tvir_MIN, mu_for_Ts),  M_MIN_WDM)) < 1e-15 ){
 ...
 if (TK_at_Z_HEAT_MAX > 0)
  ...
  while (zp > REDSHIFT){
	...
	// New in v1.3(?) : Check this part should be modified.
	...
	//New in v1.3(?): changed from FgtrM_st(zp,M_MIN_at_zp) to FgtrM_st_SFR(zp, M_MIN, M_TURN, ALPHA_STAR, ALPHA_ESC)
	//***********************************************************************
	//***********************************************************************
	//   CHECK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	//***********************************************************************
	if (HII_EFF_FACTOR_SFR == 1) {
	  M_MIN_at_zp = M_TURN/50; // Does this physically make sense?
      filling_factor_of_HI_zp = 1 - HII_EFF_FACTOR * FgtrM_st_SFR(zp, M_MIN_at_zp, M_TURN, ALPHA_STAR, 0.) / (1.0 - x_e_ave);
      fprintf(stderr, "z'=%f, filling factor of HI is %e, without x_e would be %e, time=%06.2f min\n",
	      zp,filling_factor_of_HI_zp,1 - HII_EFF_FACTOR * FgtrM_st_SFR(zp,M_MIN_at_zp,M_TURN,ALPHA_STAR,0.),(double)clock()/CLOCKS_PER_SEC/60.0);
	}
	else {
	...
	prev_R = R_values[R_ct-1];
	  ...
	  // New in v1.3(?): TtoM(..) has to be changed to M_TURN/50 ?
      sigma_Tmin[R_ct] = sigma_z0(FMAX(TtoM(zpp, X_RAY_Tvir_MIN, mu_for_Ts),  M_MIN_WDM));

	// New in v1.3(?): To calculate collapse fraction for new parametrization
	if (HII_EFF_FACTOR_SFR != 0) {
	  initialiseGL_FcollSFR(NGL_SFR,FMAX(TtoM(zpp, X_RAY_Tvir_MIN, mu_for_Ts),M_MIN_WDM),RtoM(R_values[R_ct]));
	  initialiseFcollSFR_spline(zpp,FMAX(TtoM(zpp, X_RAY_Tvir_MIN, mu_for_Ts),M_MIN_WDM),RtoM(R_values[R_ct]),M_TURN,ALPHA_STAR,0);
	  //initialiseGL_FcollSFR(NGL_SFR,M_MIN,RtoM(R_values[R_ct]));
	  //initialiseFcollSFR_spline(zpp,M_MIN,RtoM(R_values[R_ct]),M_TURN,ALPHA_STAR,0);
	}
	...
	// New in v1.3(?)
	if (HII_EFF_FACTOR_SFR !=0) {
	  FcollSpline_SFR(delNL0[R_ct][box_ct], &(Splined_Fcoll));
	  ...
	  if (HII_EFF_FACTOR_SFR !=0) {
	    ST_over_PS[R_ct] = FgtrM_st_SFR(zp,M_MIN_at_zp,M_TURN,ALPHA_STAR,0.) / fcoll_R;
	  }
	  else {
      ...
      printf("ST/PS=%g, mean_ST=%g, mean_ps=%g\n, ratios of mean=%g\n", ST_over_PS[R_ct], 
	     FgtrM_st_SFR(zp,M_MIN_at_zp,M_TURN,ALPHA_STAR,0.), 
	     FgtrM(zpp, TtoM(zpp, X_RAY_Tvir_MIN, mu_for_Ts)),
	     FgtrM_st_SFR(zp,M_MIN_at_zp,M_TURN,ALPHA_STAR,0.)/FgtrM(zpp, TtoM(zpp, X_RAY_Tvir_MIN, mu_for_Ts))
	     );
		}
		else {
      printf("ST/PS=%g, mean_ST=%g, mean_ps=%g\n, ratios of mean=%g\n", ST_over_PS[R_ct], 
	  ...
	  // New in v1.3(?)
	  if (HII_EFF_FACTOR_SFR != 0) {
      lower_int_limit = FMAX(nu_tau_one(zp, zpp, x_e_ave, filling_factor_of_HI_zp, M_TURN, ALPHA_STAR), NU_X_THRESH);
	  }
	  else {
      lower_int_limit = FMAX(nu_tau_one(zp, zpp, x_e_ave, filling_factor_of_HI_zp, 0., 0.), NU_X_THRESH);
	  }
