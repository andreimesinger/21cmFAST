...
int main(int argc, char ** argv){
  ...
  while (Z >= ZLOW){
    ...
    case 2:
      if (USE_HALO_FIELD)
	  // New in v1.4
	  // These file names are just a stopgap to avoid error. 
	  // I have to modify this part properly, since I just relaced EFF_FACTOR_PL_INDEX with HII_EFF_FACTOR.
	...
	sprintf(cmnd, "./delta_T %06.2f ../Boxes/xH_z%06.2f_nf*_eff%.1f_effPLindex%.1f_HIIfilter%i_Mmin%.1e_RHIImax%.0f_%i_%.0fMpc ../Boxes/Ts_z%06.2f_zetaX%.1e_alphaX%.1f_TvirminX%.1e_*Pop%i_%i*_%.0fMpc", Z, Z, HII_EFF_FACTOR, HII_EFF_FACTOR, HII_FILTER, M_MIN, R_BUBBLE_MAX, HII_DIM, BOX_LEN, Z, ZETA_X, X_RAY_SPEC_INDEX, X_RAY_Tvir_MIN, Pop, HII_DIM, BOX_LEN);
      else
