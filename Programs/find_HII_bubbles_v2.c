...
int parse_arguments_SFR(int argc, char ** argv, int * num_th, int * arg_offset, float * F_STAR10,
  ...
  *REDSHIFT = atof(argv[*arg_offset+1]);
  if (INHOMO_RECO){
   ...
   fprintf(stderr, "find_HII_bubbles: command line parameters are as follows\nnum threads=%i, f_star10=%g, alpha_satr=%g, f_esc10=%g, alpha_esc=%g, Mdrop=%g, mfp=%g Mpc, z=%g, prev z=%g\n",
	  *num_th, *F_STAR10, *ALPHA_STAR, *F_ESC10, *ALPHA_ESC, *M_DROP, *MFP, *REDSHIFT, *PREV_REDSHIFT);
  ...
  *error_message = '\0';
  ...
  if (HII_EFF_FACTOR_SFR != 0){
    if( !parse_arguments_SFR(argc, argv, &num_th, &arg_offset, &F_STAR10, &ALPHA_STAR, &F_ESC10,
		&ALPHA_ESC, &M_DROP, &MFP, &REDSHIFT, &PREV_REDSHIFT)){
...
Check that your inclusion (or not) of [<previous redshift>] is consistent with the INHOMO_RECO flag in ../Parameter_files/ANAL_PARAMS.H\nAborting...\n");
  ...
  printf("USE SFR Parametrization\n");
  printf("M_MIN = %16.4e\n",M_MIN);
    //M_MIN = M_DROP/50.;
	TVIR_MIN = 1e4;
    if (TVIR_MIN < 9.99999e3){ // neutral IGM
...
//printf("TEST: PL\n");
			...
			/*if(density_over_mean-1 > 1.6){
			 test1 = FgtrConditionalM_SFR(REDSHIFT,log(M_MIN),log(massofscaleR),Deltac,density_over_mean-1.,M_DROP,ALPHA_STAR,ALPHA_ESC);
			 test2 = FgtrConditionalM_SFR(REDSHIFT,log(M_MIN/10.),log(massofscaleR),Deltac,density_over_mean-1.,M_DROP,ALPHA_STAR,ALPHA_ESC);
			 ratio1 = (Splined_Fcoll-test1)/test1*100.;
			 ratio2 = (Splined_Fcoll-test2)/test2*100.;
			 if(ratio1 > 0.5 || ratio2 > 0.5) {
			 printf("\n x,y,z = %d, %d, %d\n",x,y,z);
			 printf("delta = %16.4e\n",density_over_mean-1);
			 printf("exact = %16.4e, exact 1/10 %16.4e\n", test1,test2);
			 printf("interpolate = %16.4e\n",Splined_Fcoll);
			 printf("ratio = %f %f\n", ratio1, ratio2);
			 }
			}*/
		  }
