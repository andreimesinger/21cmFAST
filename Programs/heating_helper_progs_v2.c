...
float M_MIN_at_z, M_MIN_at_zp;
 /* New in v1.3(?): This is for test to find Mmin which is the same with the original one. 
...
// New in v1.3(?)
double tauX(double nu, double x_e, double zp, double zpp, double HI_filling_factor_zp, float M_TURN, float ALPHA_STAR);
...
// New in v1.3(?)
double nu_tau_one(double zp, double zpp, double x_e, double HI_filling_factor_zp, float M_TURN, float ALPHA_STAR); 
...
void evolveInt(float zp, float curr_delNL0[], double freq_int_heat[], 
  ...
  float Splined_Fcoll; //New in v1.3(?)
 /* New in v1.3(?): This is for test to find Mmin which is the same with the original one. 
    If the code works, you don't need this parameter.
 */
float mu_for_Ts; 
...
/* Test parameters end */
  ...
  for (zpp_ct = 0; zpp_ct < NUM_FILTER_STEPS_FOR_Ts; zpp_ct++){
	...
	if (HII_EFF_FACTOR_SFR != 0) {
	  ...
	  initialiseFcollSFR_spline(zpp,FMAX(TtoM(zpp, X_RAY_Tvir_MIN, mu_for_Ts),M_MIN_WDM),RtoM(R_values[zpp_ct]),M_TURN,ALPHA_STAR,0);
	  //initialiseGL_FcollSFR(NGL_SFR, M_MIN, RtoM(R_values[zpp_ct]));
	  //initialiseFcollSFR_spline(zpp, M_MIN,RtoM(R_values[zpp_ct]),M_TURN,ALPHA_STAR,0);
	  FcollSpline_SFR(curr_delNL0[zpp_ct], &(Splined_Fcoll));
...
typedef struct{
  ...
  // New in v1.3(?)
  double nu_0, x_e, ion_eff,M_TURN,ALPHA_STAR;
} tauX_params;
double tauX_integrand(double zhat, void *params){
  ...
  // New in v1.3(?)
  if (HII_EFF_FACTOR_SFR != 0) {
    //fcoll = FgtrM_st_SFR(zhat, M_TURN/50, M_TURN, ALPHA_STAR, 0.);
    fcoll = FgtrM_st_SFR(zhat, get_M_min_ion(zhat), p->M_TURN, p->ALPHA_STAR, 0.);
  }
  else {
  ...
  double result, error, fcoll;
	 ...
	 printf("in taux, parameters are: %e, %e, %f, %f, %e\n", nu, x_e, zp, zpp, HI_filling_factor_zp);
       */
       ...
       if (HII_EFF_FACTOR_SFR != 0) {
         //fcoll = FgtrM_st_SFR(zhat, M_TURN/50, M_TURN, ALPHA_STAR, 0.);
         fcoll = FgtrM_st_SFR(zp, M_MIN_at_zp, M_TURN, ALPHA_STAR, 0.);
       }
       else {
...
typedef struct{
  ...
  // New in v1.3(?)
  double x_e, zp, zpp, HI_filling_factor_zp, M_TURN, ALPHA_STAR;
} nu_tau_one_params;
double nu_tau_one_helper(double nu, void * params){
  ...
  // New in v1.3(?)
  return tauX(nu, p->x_e, p->zp, p->zpp, p->HI_filling_factor_zp, p->M_TURN, p->ALPHA_STAR) - 1;
}
//double nu_tau_one(double zp, double zpp, double x_e, double HI_filling_factor_zp){
// New in v1.3(?)
double nu_tau_one(double zp, double zpp, double x_e, double HI_filling_factor_zp, float M_TURN, float ALPHA_STAR){
  ...
  // New in v1.3(?)
  if (tauX(HeI_NUIONIZATION, x_e, zp, zpp, HI_filling_factor_zp, M_TURN, ALPHA_STAR) < 1)
